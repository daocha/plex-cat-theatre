<div align="center">
  ...
  <p><strong>超輕量，隱私模式 🔐，跨裝置，智慧串流</strong></p>
  <p>不需要應用程式。伺服器安裝簡單，介面對手機友善，讓你的 NAS 在任何地方都能連線，並可選擇整合 Plex</p>
  <p>
...
> 沒有笨重依賴，一切都很透明。以 Flask、Waitress 和 `ffmpeg` 建構的輕量級自架電影瀏覽與串流伺服器，並可選擇整合 _`Plex`_，以提供更著重相容性的播放方案。
...
- 📽️ 混合播放策略：直接播放、針對 `.mkv` 與 `.ts` 的內建本機轉碼，或由 Plex 支援的 HLS 代理。可依媒體輕鬆切換
...
<table>
  ...
  <tr>
    ...
    <td>視為私密的資料夾前綴。例如 <code>Personal</code>。在你從介面解鎖前，位於 <code>Personal</code> 資料夾下的內容都會被鎖住。</td>
  </tr>
  <tr>
    <td><code>private_passcode</code></td>
    <td>私密模式口令雜湊，不應直接以明文修改。若要更新，請參考 <code>更改口令</code> 章節。</td>
  </tr>
  <tr>
  ...
  <td>📍 [可選] 啟用 Plex 整合。預設值：<code>false</code>。啟用前請先確認 Plex Server 已正確安裝並設定完成。<br> 這個伺服器支援原生字幕，但若你想自動抓取字幕，通常更建議交給 Plex 處理。
  <br> 如果你希望獲得更好的隨選轉碼體驗，強烈建議安裝 Plex server，以提供更順暢的媒體串流。<br>
  即使沒有 Plex server，這個服務仍然可以正常運作，但請留意：
  <br>→ 對於你的裝置可以直接播放的媒體，拖曳定位功能可正常使用。
  <br>→ 對於你的裝置無法直接播放的媒體，例如 <code>h.265 搭配 DTS 音訊</code>（h.265 搭配 AAC 或 MP3 不受影響）、<code>.mkv</code>、<code>.ts</code> 或 <code>.wmv</code>，這個服務仍可即時轉碼，但拖曳定位可能無法使用。
  </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>包含 <code>enabled</code> 與 <code>audio_whitelist</code> 的物件。當 <code>enabled=true</code> 時，可使用原生播放器直接播放媒體而不必轉碼（速度較快）。建議保留預設設定。</td>
  </tr>
</table>
...
- 現有的本地快取縮圖仍可重複利用
- 預覽影格產生仍會維持啟用
- Plex 整合仍然是可選的，純本機模式依然可用

### → 如何取得 Plex 權杖

#### 方法 1：既有 Plex Web 工作階段
...
如果你已在同一台機器上登入 Plex Web 且工作階段仍有效，可在 DevTools 中檢查 Plex 請求並尋找：
...
當 Plex 未啟用，或你刻意想維持完全本機時，就會走這個回退路徑。
...
如果自訂介面可從遠端連線，但 Plex 只能在私有 LAN 中存取，那麼 movies server 所在主機仍必須能直接連到 Plex 後端。
...
從可連到 Plex 的 Tailscale 節點宣告路由：

```bash
sudo tailscale up --advertise-routes=192.168.50.0/24
```

接著從 movies server 主機確認可達性：
...
- 介面的反向代理或 MagicDNS 名稱本身不會讓 Plex 自動變得可達
...
`Rescan` 按鈕不會立即啟動增量掃描，而是先開啟操作對話框。
...
若已設定 `mount_script`，且某個媒體請求命中遺失的資料夾，伺服器會：
...
前端會將播放過程中的 404 視為本次嘗試的終止條件，並顯示重試訊息，而不是持續重複打到伺服器。

---

## 📄 產生的檔案

以下檔案會在執行時產生，不應提交到版本庫：
...
- 目前應用會直接從 `index.html` 載入 `movies.js`，因此前端變更不需要重新建置 `movies.min.js` 就會生效。
- 先正常重新整理頁面
- 若 JS bundle 已變更，請確認 `index.html` 引用的是預期版本的 bundle

### → 私密內容的直接播放失敗

- 請重新解鎖私密模式，以刷新 `movies_device_id` Cookie

### → Plex 播放失敗但直接播放正常

- 確認 movies server 主機可以連到 `plex.base_url`
- 確認設定中已啟用 Plex
- 確認設定的 token 有效
...
- 可持續為這些檔案啟用 Plex，或透過本機轉碼 / Plex 強制走相容路徑
...
本專案以 MIT License 發布。發布或重新散布時，請附上包含 MIT 文字的 `LICENSE` 檔案。
