...
> 以 Flask、Waitress 和 `ffmpeg` 建構的輕量級自託管電影瀏覽與串流伺服器，並可選擇整合 _`Plex`_，以提供更著重相容性的播放方案。
...
[English](./README.md) | [简体中文](./README.zh-CN.md) | `繁體中文（香港）` | [繁體中文（台灣）](./README.zh-TW.md) | [Français](./README.fr.md) | [한국어](./README.ko.md) | [日本語](./README.ja.md) | [Deutsch](./README.de.md) | [ไทย](./README.th.md) | [Tiếng Việt](./README.vi.md) | [Nederlands](./README.nl.md)
...
- 縮圖與預覽影格產生
- 以裝置為基礎的私人資料夾存取控制
- 透過如 `/movie/` 這類路徑前綴在反向代理後方部署
- 混合播放策略：直接檔案播放、內建本地轉碼，或 Plex HLS 播放
...
若你將應用透過反向代理部署於如 `/movie/` 這類前綴下，請改為開啟帶前綴的 URL。
...
- `private_folder`：視為私人的資料夾前綴
- `private_passcode`：私人模式密碼雜湊
- `mount_script`：播放遇到遺失媒體資料夾時可呼叫的掛載腳本
- `transcode`：為 `.mkv`、`.ts` 等來源容器啟用目錄掃描側的背景轉碼工作程序；這可能會在媒體庫旁邊產生額外的轉碼檔案，因此通常建議保持為 `false`，尤其在啟用 Plex 整合時
- `auto_scan_on_start`：啟動時重新掃描媒體
- `on_demand_transcode`：為播放器執行期間啟用來源容器轉碼，可優先使用硬件編碼，不可用時再回退至軟件編碼
...
這是未啟用 Plex 時的後備路徑，或你刻意希望完全維持本地方案時的路徑。

目前實作：

- `.mkv` 和 `.ts` 可透過 `/hls/<id>/index.m3u8` 作為本地 HLS 提供
- 同一類檔案亦可透過 `/video/<id>?fmp4=1` 以 fragmented MP4 方式串流
- HLS 分段按需以 `ffmpeg` 產生
- 可先嘗試硬件編碼，再回退到 `libx264`
...
這樣拆分的原因是，原生 `<video src="...">` 請求無法附帶任意自訂標頭。
...
如果自訂 UI 可從遠端存取，但 Plex 僅能在私人 LAN 內存取，則 movies server 主機仍必須能直接連到 Plex 後端。
...
- UI 的反向代理或 MagicDNS 名稱本身不會令 Plex 自動可達
...
- 過期項目會在讀取時或後續清理時刪除
- 儲存新快照後會執行清理
- 瀏覽器儲存壓力或手動清除網站資料亦可能移除 IndexedDB 資料

---

## 掃描行為

目錄掃描的設計是即使仍要走訪每個設定根目錄，成本亦可維持增量化。
...
目前 `index.html` 直接載入 `movies.js`，因此前端修改無需重新建構 `movies.min.js` 亦會生效。
...
- 先正常重新整理頁面
- 若 JS 套件已變更，確認 `index.html` 指向預期的套件版本

### 私人內容的直接播放失敗

- 重新解鎖私人模式，讓 `movies_device_id` Cookie 重新整理
...
- 對這些檔案保持啟用 Plex，或透過本地轉碼/Plex 強制走相容路徑
...
本專案以 MIT License 發佈。若你打算發佈或重新發佈它，請加入包含 MIT 內容的 `LICENSE` 檔案。
